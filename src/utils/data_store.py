...
class OrderDataStore:
    ...
    def list_products(
        ...
        *,
        ...
        for product in self.products:
            ...
            haystack = _normalize(searchable_text)
            score = 0
            matched_terms = []
            ...
            for term in query_terms:
                if term in haystack:
                    score += 4 if any(
                        term in _normalize(part)
                        for part in [product.name, product.sku, product.product_id]
                    ) else 2
            ...
            results.append(
                (
                    ...
                    product.product_id,
                    {
                        ...
                        "next_step": "Call get_product_details with the final product_id list before pricing or saving.",
                    },
                )
            )

        results.sort(key=lambda item: (-item[0], self.product_index[item[2]].unit_price, item[2]))
        return [item[-1] for item in results[: max(1, min(limit, 20))]]
    ...
    def get_product_details(self, product_ids: list[str]) -> list[dict]:
        ...
        return {
            "status": "ok" if len(found_product_ids) == len(product_ids) and found_product_ids else "error",
            "detail_token": self.build_detail_token(found_product_ids) if found_product_ids else "",
            "items": details,
            "instruction": "If requested quantity exceeds stock, stop and do not call discount, calculate_order_totals, or save_order.",
        }
        ...
        *,
        ...
        payload = {
            ...
            "discount": {
                ...
                "customer_tier": (customer_tier or "standard").strip().lower(),
            },
